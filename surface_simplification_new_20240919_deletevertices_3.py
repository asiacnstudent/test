...
def plot_mesh_clusters(mesh,n_vertices_to_merge):  # 可视化聚类结果
  ...
  for i in range(num_clusters):
    ...
    if total_weight_sum == 0:
      raise ValueError("Total weight sum 为零，会导致除零错误。")
    if tasks_per_cluster[i] == 0 or total_weight_sum == 0:
      tasks_per_cluster[i] = 0  # 或根据需要进行处理
    else:
      tasks_per_cluster[i] = int((tasks_per_cluster[i] / total_weight_sum) * n_vertices_to_merge)
...
def add_pair(mesh, v1, v2, edge_connection):    # 将一对顶点添加到堆中
  ...
  # 计算最终 cost
  alpha = 1.0  # QEM 误差权重
  beta = 10.0  # 质量因子权重（调整后可提高狭长三角形的折叠优先级）
  cost = alpha * cost + beta * (1 - min_quality)
...
if __name__ == '__main__':
  #run_all()
  #run_bunny_many()
  run_one(1)      #  调用run_one函数，参数为n，即运行第n+1个模型的简化