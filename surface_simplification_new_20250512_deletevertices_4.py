...
def get_mesh(idx=0):
  ...
  if idx == -1:
    ...
    mesh_fns = [['meshes/bunny2.off',        4769],
                ['meshes/cat.off',           5000],
...
def simplify_mesh(mesh_orig, n_vertices_to_merge):
    ...
    tb = time.time()
    for cluster_idx, stat in cluster_stats.items():
        if stat['frozen']:
            continue
        cluster_vertices = stat['vertex_indices']
        task_count = stat['assigned_tasks']
        select_vertex_pairs(mesh, cluster_vertices)  # 将该类所有顶点对入堆
        for _ in tqdm(range(task_count)):
            contract_best_pair(mesh)
            if mesh['n_faces'] <= MINIMUM_NUMBER_OF_FACES:
                break
        clear_heap(mesh)  # 清堆，为下一个类别准备
...
def perform_clustering_and_task_assignment(mesh, n_vertices_to_merge, n_clusters=8):
    ...
    for cid, stat in cluster_stats.items():
        ...
        w_i = alpha * w_low + (1 - alpha) * w_v   #任务分配公式
        S_i = int(w_i * n_vertices_to_merge)
        stat['assigned_tasks'] = S_i
        stat['error_threshold'] = stat['mean_abs_K'] * (S_i / n_vertices_to_merge) * scaling_factor
    ...
    for cid, stat in cluster_stats.items():
        print(f"Cluster {cid:2d} | Vertices: {stat['vertex_count']:4d} | Faces: {stat['face_count']:4d} | Assigned tasks: {stat['assigned_tasks']:4d} | Mean |K|: {stat['mean_abs_K']:.6f}")
...
def update_cluster_after_merge(mesh, v1, v2, cost):
    ...
    mesh['actual_merge_count'] += 1  # 增加全局统计
    stat['cumulative_error'] += cost

    if (stat['actual_simplified'] >= stat['assigned_tasks'] or
        stat['cumulative_error'] >= stat['error_threshold']):
        stat['frozen'] = True
        leftover = stat['assigned_tasks'] - stat['actual_simplified']
        if leftover > 0:
            redistribute_remaining_tasks(mesh, leftover)
...
def redistribute_remaining_tasks(mesh, extra_tasks):
    active = [cid for cid, s in mesh['cluster_stats'].items() if not s['frozen']]
    if not active:
        return
    total_v = sum([mesh['cluster_stats'][cid]['vertex_count'] for cid in active])
    for cid in active:
        ratio = mesh['cluster_stats'][cid]['vertex_count'] / total_v
        mesh['cluster_stats'][cid]['assigned_tasks'] += int(extra_tasks * ratio)
...
def add_pair(mesh, v1, v2, edge_connection):    # 将一对顶点添加到堆中
  ...
  # 计算最终 cost
  alpha = 1  # QEM 误差权重
  beta = 0  # 质量因子权重（调整后可提高狭长三角形的折叠优先级）
...
def contract_best_pair(mesh):
    ...
    if mesh['cluster_stats'][cid_v1]['frozen'] and mesh['cluster_stats'][cid_v2]['frozen']:
        return
